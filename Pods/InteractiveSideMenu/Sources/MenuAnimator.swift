...
//                                                       action: #selector(MenuInteractiveTransition.handlePanDismission(recognizer:)))
            ...
            //christeena updated for right menu : begin
            let newScale = 1 - (1 - options.contentScale)
            var newX = screenWidth - options.visibleContentWidth
            
            guard let contentSnapshotView = self.contentSnapshotView else {
                fatalError("Invalid `contentSnapshotView` value. This property should not be nil")
            }
            newX = -(contentSnapshotView.frame.size.width * newScale - options.visibleContentWidth)
            
            let newOrigin = CGPoint(x: newX /*screenWidth - options.visibleContentWidth*/, y: toViewController.view.frame.origin.y)
            //end
    ...
    func cancelTransition(currentPercentComplete : CGFloat) {
        //christeena.john
        print("**** In cancelTransition")
        let percentCompletion = self.present ? 1.0 : 0
        //end
        transitionStarted = false

        let animation : () -> Void = { [unowned self] in self.updateTransition(percentComplete:CGFloat(percentCompletion) /*0*/) }
        let completion : (Bool) -> Void = { [unowned self] _ in
            guard let transitionContext = self.transitionContext else {
                fatalError("Invalid `transition.transitionContext` value. This property should not be nil")
            }
    ...
    func handlePan(recognizer: UIPanGestureRecognizer) {
        ...
        //christeena !present changing to present
        if present {
            velocity = -velocity
        }
    ...
    func updateTransition(percentComplete: CGFloat) {
        ...
        var totalWidth = screenWidth - options.visibleContentWidth
        ...
        if present {
            ...
            //christeena updated for right menu : begin
            totalWidth = -(contentSnapshotView.frame.size.width*newScale - options.visibleContentWidth)
            //end
            ...
            //christeena updated
            let newOrigin = CGPoint(x: newX, y: newX/*toViewController.view.frame.origin.y*/)
            let rect = CGRect(origin: newOrigin, size: toViewController.view.frame.size)
